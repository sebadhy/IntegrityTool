...
# BD y notificaciones — opcionales: la app funciona aunque fallen
try:
    import db as _db
    _DB_AVAILABLE = True
except Exception:
...
def _load_corpus_from_db() -> dict:
    """
    Carga el corpus histórico desde BD y devuelve corpus_payload compatible con app.py.
    Si la BD falla o está vacía, devuelve payload vacío.
    """
    from src.analyzer.review_synthesis import build_corpus_context

    try:
        with _db.get_connection() as conn:
            data = _db.get_corpus_analysis_from_db(conn)
    except Exception as exc:
        st.warning(f"No se pudo cargar corpus desde BD: {exc}. Comparación histórica desactivada.")
        return empty_corpus_payload()

    n = data["total_processes"]
    context = build_corpus_context(data["analysis"], n) if n > 0 else {}

    if n > 0:
        st.info(f"Corpus histórico: **{n}** proceso(s) cargados desde base de datos.")
    else:
        st.info("El corpus histórico en BD está vacío. Cada análisis que realice se irá agregando automáticamente.")
...
def _autosave_to_corpus(
    ...
    """
    Guarda el documento analizado en el corpus histórico de la BD.
    Solo lo hace una vez por documento (tracked por session_state).
    """
    if not _DB_AVAILABLE:
        return

    save_key = f"corpus_saved_{sig}"
    ...
    nivel = (ai_brief or {}).get("overall_attention_level")

    try:
        with _db.get_connection() as conn:
            _db.upsert_corpus_documento(
                conn,
                process_id=process_id,
                document_name=document_name,
                hallazgos_df=enriched_df,
                nivel_atencion=nivel,
            )
        st.session_state[save_key] = True
    except Exception as exc:
        import logging
        logging.getLogger(__name__).warning("No se pudo guardar en corpus: %s", exc)


def render_recipient_management() -> None:
    """Gestión de destinatarios de alertas en el sidebar."""
    st.sidebar.divider()
    st.sidebar.subheader("Destinatarios de alertas")

    if not _DB_AVAILABLE:
        st.sidebar.caption("BD no disponible. Instale pyodbc y configure DB_* en .env")
        return

    try:
        with _db.get_connection() as conn:
            recipients = _db.get_all_recipients(conn)
    except Exception as exc:
        st.sidebar.warning(f"Sin conexión a BD: {exc}")
        return

    if recipients:
        for r in recipients:
            col_info, col_btn = st.sidebar.columns([3, 1])
            label = r["nombre"] or r["email"]
            icon = "✓" if r["activo"] else "✗"
            col_info.caption(f"{icon} {label}")
            if r["activo"]:
                if col_btn.button("−", key=f"del_{r['id']}", help="Desactivar"):
                    with _db.get_connection() as conn:
                        _db.remove_recipient(conn, r["email"])
                    st.rerun()
    else:
        st.sidebar.caption("Sin destinatarios. Agregue uno abajo.")

    with st.sidebar.form("form_add_recipient", clear_on_submit=True):
        new_email = st.text_input("Email", placeholder="revisor@entidad.gob.ec")
        new_nombre = st.text_input("Nombre (opcional)")
        if st.form_submit_button("Agregar destinatario"):
            if "@" in new_email:
                try:
                    with _db.get_connection() as conn:
                        _db.add_recipient(conn, new_email.strip(), new_nombre.strip())
...
def render_notification_panel(
    ...
    """
    Panel de alerta y envío de notificación.
    Se muestra siempre si hay hallazgos; con banner destacado si el nivel es Alto.
    """
    if enriched_df.empty:
        return

    # Determinar nivel de alerta
    alto_count = int((enriched_df.get("atención sugerida") == "Alto").sum()) if "atención sugerida" in enriched_df.columns else 0
    ai_level = (ai_brief or {}).get("overall_attention_level", "")
    is_high = ai_level == "Alto" or alto_count >= 2

    # Clave de sesión ligada al documento actual
    sig = str(st.session_state.get("upload_signature", ""))
    notif_key = f"notif_result_{sig}"

    if is_high:
        st.markdown(
            """<div style="background:#fef2f2; border:2px solid #fca5a5; border-radius:6px;
                          padding:14px 18px; margin:16px 0;">
                <span style="color:#dc2626; font-weight:bold; font-size:15px;">
                    Nivel de atención Alto detectado
                </span>
                <p style="margin:6px 0 0; color:#7f1d1d; font-size:13px;">
                    Este documento presenta señales que sugieren revisión prioritaria.
                    Use el botón para notificar a los revisores configurados en la base de datos.
                </p>
            </div>""",
            unsafe_allow_html=True,
        )

    with st.expander("Notificar a revisores" + (" — ALERTA ALTA" if is_high else ""), expanded=is_high):
        if not _DB_AVAILABLE:
            st.warning("La base de datos no está disponible. Configure pyodbc y DB_* en .env para habilitar notificaciones.")
            return
        if not _NOTIFIER_AVAILABLE:
            st.warning("El módulo de notificaciones no está disponible.")
            return

        # Mostrar destinatarios activos
        try:
            with _db.get_connection() as conn:
                active_recipients = _db.get_active_recipients(conn)
        except Exception as exc:
            st.error(f"No se pudo conectar a la BD: {exc}")
            return

        if active_recipients:
            st.caption(f"Destinatarios activos: {', '.join(active_recipients)}")
        else:
            st.warning("No hay destinatarios configurados. Agregue correos en el panel lateral.")
            return
        ...
        if send_clicked:
            top_findings = (
                enriched_df[enriched_df["atención sugerida"] == "Alto"].head(3).to_dict("records")
                if "atención sugerida" in enriched_df.columns
                else []
            )
            brief_for_email = ai_brief or {
                "overall_attention_level": ai_level or ("Alto" if is_high else "Medio"),
                "document_summary": f"Análisis de {document_name}: {len(enriched_df)} señales detectadas, {alto_count} de atención alta.",
                "main_review_topics": list(enriched_df["tema de revisión"].unique()[:5]) if "tema de revisión" in enriched_df.columns else [],
            ...
            with st.spinner("Enviando alerta…"):
                # Guardar en BD
                db_ok, db_msg = _db.save_pipeline_results(document_name, ai_brief, enriched_df)
                # Enviar email
                email_ok, email_msg = _notifier.send_alert_email(
                    document_name, brief_for_email, top_findings, active_recipients
                )
        ...
        # Mostrar resultado del último envío
        result = st.session_state.get(notif_key)
        if result:
            with status_col:
                if result["email_ok"]:
                    st.success(f"Email: {result['email_msg']}")
                else:
                    st.error(f"Email: {result['email_msg']}")
            if result.get("db_ok"):
                st.caption(f"BD: {result['db_msg']}")
            elif result.get("db_msg"):
                st.caption(f"BD (sin guardar): {result['db_msg']}")


def render_llm_test_button() -> None:
    st.sidebar.title("Configuración técnica")
    st.sidebar.markdown(
        '<a href="/Ayuda" target="_blank" style="'
        'display:block; background:#1e3a5f; color:white; text-align:center;'
...
def render_review_flow() -> None:
    ...
    if use_historical_corpus:
        if _DB_AVAILABLE:
            corpus_payload = _load_corpus_from_db()
        else:
    ...
    _llm_configured = bool(os.getenv("OPENAI_API_KEY") or os.getenv("GROQ_API_KEY"))
    if enable_ai_reading and not _llm_configured:
        st.warning(
            "IA generativa no configurada. Se muestran resultados basados en reglas "
            "y comparación documental."
        )
    ...
    _doc_sig = str(st.session_state.get("upload_signature", ""))
    _autosave_to_corpus(_doc_sig, uploaded_file.name, enriched_df, ai_brief)
    render_notification_panel(uploaded_file.name, enriched_df, ai_brief)
...
# Ocultar la navegación automática de páginas (multi-page Streamlit)
st.markdown(
